...
- Weiteres Feedback zu Todos umgesetzt: Datumsangabe bei "Frühere"/"Erledigt" wieder entfernt (unnötig), dafür lassen sich frühere, noch offene Todos jetzt direkt zu "Heute" verschieben — per Wisch-Aktion (Mobile) bzw. eigenem Button (Desktop), gleiches Icon wie der "Heute"-Tab. TodoStorageService.setDate() ergänzt, setDone()/setDate() teilen sich jetzt eine gemeinsame updateTodo()-Hilfsfunktion.

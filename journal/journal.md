...
- Notiz-Ansicht/-Bearbeitung auf Feedback hin von einer eigenen Vollbild-Seite (eigene Route, eigener Header mit Zurück-Pfeil) zu einem Bottom-Sheet-Modal über der Notizen-Liste umgebaut: nimmt nur ~85% der Höhe ein, per Ziehpunkt weiter auf-/zuziehbar, Liste bleibt im Hintergrund sichtbar. `NoteEditPage` (Route) wurde zu `NoteEditorComponent` (in die Liste eingebettetes Modal), die Routen `notiz/neu`/`notiz/:id` sind entfallen.
- Dabei zwei Bugs gefunden und behoben, die mit der alten Seiten-Variante zusammenhingen: (1) Angulars Standard-Sanitizer für `[innerHTML]` entfernt `style`-Attribute kommentarlos — die eingerückte Listen-Darstellung und die Link-Farbe griffen dadurch im echten Browser nicht, obwohl Unit-Tests grün waren (dort wird nicht sanitized). Behoben, indem `NoteContentPipe` ihr eigenes, bereits escapetes HTML explizit über `DomSanitizer.bypassSecurityTrustHtml()` als sicher markiert. (2) Nach dem Zurücknavigieren von der (inzwischen entfernten) Notiz-Seite in den Tab feuerte Ionics `ionViewWillEnter` nicht zuverlässig, weil die Seite ausserhalb des Tab-eigenen Router-Outlets lag — die Liste zeigte dadurch bis zum nächsten manuellen Neuladen einen veralteten Stand. Durch den Umbau aufs Modal (alles bleibt auf derselben Seite, kein Routenwechsel mehr) architektonisch behoben statt nur kaschiert; das Modal lädt die Liste jetzt einfach in `didDismiss` neu.
- Speichern-beim-Verlassen ist beim Modal über `canDismiss` (statt `ionViewWillLeave`) gelöst: bevor das Sheet wirklich zugeht (Fertig-Button, Wisch-Geste oder Tap auf den Hintergrund), wird noch ausstehend Getipptes gespeichert.
- Beide Bugs gezielt im echten Browser nachgestellt (nicht nur in Unit-Tests) — beide waren in den bestehenden Unit-Tests unsichtbar, siehe entsprechende Notiz vom 10.9. zu ähnlichen Browser-only-Bugs bei den Todos.
